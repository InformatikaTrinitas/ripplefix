...
export class GlobeRenderer {
  ...
  async init() {
    ...
    // ── Earth sphere ──────────────────────────────────────
    const geo = new THREE.SphereGeometry(1, 48, 48); // reduced from 64 for perf
  ...
  _buildStars(THREE) {
    const positions = [];
    for (let i = 0; i < 800; i++) { // reduced from 2000 for perf
  ...
  _applyHealthTint(THREE) {
    ...
    const h = this.health;
    let col;
    if (h > 70)      col = new THREE.Color(0x22aaff);
    else if (h > 40) col = new THREE.Color(0xff8800);
    else             col = new THREE.Color(0xff3300);
  ...
  _startLoop(THREE) {
    // Throttled to 30fps to reduce GPU load
    let lastT = 0;
    const STEP = 1000 / 30;
    const animate = (t) => {
      this._animId = requestAnimationFrame(animate);
      if (t - lastT < STEP) return;
      lastT = t;

      if (this.autoRotate && !this._dragging) {
        if (this._earth)  this._earth.rotation.y  += 0.002;
        if (this._clouds) this._clouds.rotation.y += 0.0024;
        if (this._glow)   this._glow.rotation.y   += 0.0013;
      }

      if (this._clouds) this._clouds.rotation.x += 0.0004;
...
export class MiniGlobe {
  ...
  _animate() {
    ...
    const loop = (t) => {
      ...
      if (t - lastT < STEP) return;
      lastT = t;
      if (this._mesh) this._mesh.rotation.y += 0.02;
      this._renderer.render(this._scene, this._camera);
    };
